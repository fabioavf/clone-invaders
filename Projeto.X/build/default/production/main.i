...
struct Object {
    _Bool isAlive, projectileAlive, state;
    int x, y, projX, projY;
};
# 42 "main.c" 2
...
unsigned char screenBuffer[8 / 2][16];
...
void main(void) {
    char customChars[64] = {
        0x00, 0x00, 0x00, 0x00, 0x00, 0x04, 0x0E, 0x1B,
        0x00, 0x04, 0x04, 0x00, 0x00, 0x04, 0x0E, 0x1B,
        0x00, 0x00, 0x00, 0x00, 0x00, 0x04, 0x04, 0x00,
        0x00, 0x04, 0x04, 0x00, 0x00, 0x00, 0x00, 0x00,
        0x0A, 0x15, 0x0E, 0x11, 0x00, 0x00, 0x00, 0x00,
        0x0A, 0x15, 0x0E, 0x0A, 0x00, 0x00, 0x00, 0x00,
        0x0A, 0x15, 0x0E, 0x11, 0x00, 0x04, 0x04, 0x00,
        0x0A, 0x15, 0x0E, 0x0A, 0x00, 0x04, 0x04, 0x00,
    };
    ...
    for (i = 0; i < 16; i++) {
        if (aliens[i].projectileAlive) {
            if ((aliens[i].x == aliens[i].projX) && (aliens[i].projY == 1) && (aliens[i].isAlive) && i < 8) {
                ...
                screenBuffer[aliens[i].projY / 2][aliens[i].projX] = 1;
            } else {
                screenBuffer[aliens[i].projY / 2][aliens[i].projX]
                        = (aliens[i].projY % 2) ? 2 : 3;
            }
        }
    }

    if (player.isAlive) {
        screenBuffer[player.y / 2][player.x] = 0;
    }

    if (player.projectileAlive) {
        if (player.projY == (player.y - 1)) {
            screenBuffer[player.projY / 2][player.projX] = 1;
        } else {
...
void gameLogic() {
    ...
    if (contadorLoop == 0) {
        ...
        for (i = 0; i < 16; i++) {
            if (aliens[i].isAlive) {
                ...
                if (rand() % 100 < 8 && !aliens[i].projectileAlive) {
    ...
    if (!aliensLeft && level < 10) {
        setupLevel(level + 1);
    } else if (!aliensLeft && level == 10) {
        youWin();
    }
}
